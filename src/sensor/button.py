...
class Button(Sensor):
    ...
    def __init__(self, name, event_type, event_code, \
        ...
        # 释放回调函数
        self.on_release = on_release if callable(on_click) else None
    ...
    def event_handler(self, event):
        ...
        # 判断是否要调用数值变化的回调函数
        if self.value == ButtonEventFlag.BUTTON_CLICK and self.on_click is not None:
            self.on_click()
        elif self.value == ButtonEventFlag.BUTTON_CLICK and self.on_release is not None:
            self.on_release()
        # 数值更新事件回调
    ...
    def value(self, new_value):
        '''复制传感器数值'''
        if new_value == ButtonValue.BUTTON_CLICK:
            # 设置标志位
            self._event_flag = ButtonEventFlag.BUTTON_CLICK
        elif new_value == ButtonValue.BUTTON_RELEASE:
            # 设置标志位
